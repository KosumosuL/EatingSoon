/* http服务器部分修改于Tinyhttpd
 * Tinyhttpd 是J. David Blackstone在1999年写的一个不到 500 行的超轻量型 Http Server
 * 在符合http协议的前提下，通过子进程来运行cgi文件实现原始的动态网页
 * 英文注释来源于原作者
 * 中文注释来源于AngryHacker
 */
/*
 * 在main.cpp中将accept_request作为函数指针传递给每一个实例化的task
 * task在被线程池启动后回调此函数，但由于类成员函数指针的复杂性，未能
 * 完成将http服务器部分封装成类的操作。考虑到tinyhttpd原C代码的“面向
 * 过程”特性，直接抽调出部分函数来完成对象之间的交互。
 */
#include <stdio.h>
...
#define SERVER_STRING "Server: LucasHttpServer/0.1.0\r\n"
